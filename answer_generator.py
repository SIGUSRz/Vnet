...
class Answer_Generator():
    ...
    def train_model(self):
        img_state = tf.placeholder(tf.float32, [self.batch_size, self.img_dim])
        question_batch = tf.placeholder(tf.float32, [self.batch_size, self.max_que_length])
        label_batch = tf.placeholder(tf.int64, [self.batch_size, ])
        ...
        cross_entropy = tf.nn.sparse_softmax_cross_entropy_with_logits(logits, label_batch)
        ...
        return loss, img_state, question_batch, label_batch